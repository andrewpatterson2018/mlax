...
def GetData():
    (trainX, trainY), (testX, testY) = imdb.load_data(
        path="imdb.pkl", num_words=5000, seed=12345
    )
...
def cone(combination, _learning_rate, _epochs, _batches, _seed):
    ...
    tensorboard = buildTensorBoard(
        combination, _learning_rate, _epochs, _batches, _seed
    )
...
def buildTensorBoard(combination, _learning_rate, epochs, batches, seed):
    boardString = "logs/imdb-{0}-{1}-{2}-{3}-{4}.cpkt".format(
